...
{
    public class GetChapterPagesQuery : IRequest<ApiResponse<List<ChapterPageDto>>>
    {
    ...
    public class GetChapterPagesQueryHandler : IRequestHandler<GetChapterPagesQuery, ApiResponse<List<ChapterPageDto>>>
    {
        ...
        public GetChapterPagesQueryHandler(IChapterRepository repository, IMapper mapper, IComicValidator comicValidator, ICloudinaryService cloudinaryService, IMissionProgressNotifier missionProgressNotifier, IPublishEndpoint publishEndpoint)
        {
        ...
        public async Task<ApiResponse<List<ChapterPageDto>>> Handle(GetChapterPagesQuery request, CancellationToken cancellationToken)
        {
            var chapter = await _repository.GetChapterByIdAsync(request.Id, true);
            if (chapter == null)
            {
