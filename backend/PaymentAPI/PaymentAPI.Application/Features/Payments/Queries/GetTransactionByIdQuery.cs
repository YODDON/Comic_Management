...
{
    public class GetTransactionByIdQuery : IRequest<ApiResponse<PaymentAPI.DTOs.TransactionDto>>
    {
        public Guid Id { get; set; }
    }

    public class GetTransactionByIdQueryHandler : IRequestHandler<GetTransactionByIdQuery, ApiResponse<PaymentAPI.DTOs.TransactionDto>>
    {
        ...
        public GetTransactionByIdQueryHandler(IPaymentRepository repository, ChapterGrpc.ChapterGrpcClient chapterGrpcClient, WalletService.WalletServiceClient walletGrpcClient, IRequestClient<SubmitPurchaseCommand> purchaseClient, IOptions<TopUpSettings> topUpOptions, IOptions<BankSettings> bankOptions)
        {
        ...
        public async Task<ApiResponse<PaymentAPI.DTOs.TransactionDto>> Handle(GetTransactionByIdQuery request, CancellationToken cancellationToken)
        {
            var t = await _repository.GetTransactionByIdAsync(request.Id);
            if (t == null)
            {
